        ...
        try:
            async with aiohttp.ClientSession(loop=loop) as client:
                async with client.get('http://www.energyzueri.com/legacy-feed-converter/files/json/timeline/timeline_energybern_0.json') as resp:
                    ...
                    return ***REMOVED***"title": entry["title"], "artist": entry["artist"], "cover": entry["cover"], "youtube": entry["youtube"], "duration": duration, "progress": progress***REMOVED***
        except:
            raise
            return None
        ...
        try:
            async with aiohttp.ClientSession(loop=loop) as client:
                async with client.get('http://www.capitalfm.com/dynamic/now-playing-card/digital/') as resp:
                    ...
                    return ***REMOVED***"title": title, "artist": artist, "cover": cover, "youtube": None, "duration": None, "progress": None***REMOVED***
        except:
            raise
            return None
