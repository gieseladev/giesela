        ...
        try:
            async with aiohttp.ClientSession(loop=loop) as client:
                async with client.get('http://www.capitalfm.com/dynamic/now-playing-card/digital/') as resp:
                    ...
                    title = " ".join(soup.find_all("div", attrs=***REMOVED***"itemprop": "name", "class": "track"***REMOVED***)[
                        0].text.strip().split())
                    artist = " ".join(soup.find_all("div", attrs=***REMOVED***"itemprop": "byArtist", "class": "artist"***REMOVED***)[
                        0].text.strip().split())
                    cover = soup.find_all("img", itemprop="image")[
                        0]["data-src"]

                    return ***REMOVED***"title": title, "artist": artist, "cover": cover, "youtube": "http://www.capitalfm.com", "duration": 0, "progress": 0***REMOVED***
        except:
            raise
            return None
