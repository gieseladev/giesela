...
class MusicBot(discord.Client):
    ...
    async def update_now_playing(self, entry=None, is_paused=False):
        ...
        if entry:
            ...
            if type(entry).__name__ == "StreamPlaylistEntry" and entry.radio_station_data is not None:
                name = u'{}'.format(await player._absolute_current_song())[:128]
    ...
    @command_info("1.9.5", 1477774380, {"3.4.5": (1497616203, "Improved default help message using embeds")})
    async def cmd_help(self, channel, leftover_args):
        ...
        if command:
            ...
            em = Embed(title="GIESELA HELP",
                       ...
                       description="plz be welcum to mah new list of the **most fab** commands\nYou can always use `{0}help <cmd>` to get more detailed information on a command".format(self.config.command_prefix))
    ...
    @command_info("1.0.0", 1477180800, {"3.5.2": (1497712233, "Updated documentaion for this command")})
    async def cmd_play(self, player, channel, author, leftover_args, song_url):
        ...
        if 'entries' in info:
            ...
            self.log("Processed {} songs in {} seconds at {:.2f}s/song, {:+.2g}/song from expected ({}s)".format(
                listlen,
                self._fixg(ttime),
                ttime / listlen,
    ...
    @command_info("2.0.2", 1482252120, {"3.5.2": (1497712808, "Updated help text")})
    ...
    async def cmd_np(self, player, channel, server, message):
        ...
        if player.current_entry:
            ...
            if type(player.current_entry).__name__ == "StreamPlaylistEntry":
                if Radio.has_station_data(player.current_entry.title):
                    ...
                    if current_entry is not None:
                        ...
                        prog_str = '[{}/{}]'.format(to_timestamp(progress),
                                                    to_timestamp(length))
                ...
                if player.current_entry.radio_station_data is not None:
                    self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, "Playing radio station **{}** for {}".format(player.current_entry.radio_station_data.name, format_time(player.progress)))
                    return
                else:
                    self.server_specific_data[server]['last_np_msg'] = await self.safe_send_message(channel, 'Playing live stream: {} for {}'.format(
            ...
            if player.current_entry.spotify_track and player.current_entry.spotify_track.certainty > .6:
                ...
                prog_str = "`[{}/{}]`".format(to_timestamp(player.progress),
    ...
    @command_info("1.0.0", 1477180800, {"3.5.1": (1497706997, "Queue doesn't show the current entry anymore and shows an entry's playlist if it has one")})
    async def cmd_queue(self, channel, player):
        ...
        for i, item in enumerate(player.playlist, 1):
            ...
            #         lines.append(
            #             "            ►{}. **{}**".format(ind, sub_item["name"]))

        if not lines:
            return Response("There are no songs queued! Use `{}help` to find out how to queue something.".format(self.config.command_prefix))

        total_time = sum([entry.duration for entry in player.playlist.entries])
        lines.append(
            "\n**Total duration:** `{}`".format(format_time(total_time), True, 5, 2))
    ...
    async def playlist_builder(self, channel, author, server, player, _savename):
        ...
        while (not abort) and (not save):
            ...
            # self.log("I have {} entries in the whole list and now I'm viewing from {} to {} ({} entries)".format(
    ...
    #         playlistname, player.playlist, remove_entries=[player.current_entry], new_entries=[new_entry])
    #
    # return Response("The new title is \"{}\"".format(new_title),
    # delete_after=20)
    ...
    @command_info("1.9.5", 1479945600, {"3.3.5": (1497284850, "removed delete_after keywords which was the reason this command was broken")})
    ...
    @command_info("3.4.0", 1497533758, {"3.4.8": (1497650090, "When showing changelogs, two logs can't be on the same line anymore")})
    ...
    async def on_message(self, message):
        ...
        try:
            ...
            self.log("{0.__class__}: {0.message}".format(e))
    ...
    async def on_any_update(self, before, after):
        if before.server.id in self.online_loggers:
            ...
            if before.status != after.status:
                notification = "`{}` {} {}".format(timestamp, mem_name, {discord.Status.online: "came **online**", discord.Status.offline: "went **offline**",
                                                                         discord.Status.idle: "went **away**", discord.Status.dnd: "doesn't want to be disturbed"}[after.status])
