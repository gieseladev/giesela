...
class MusicBot(discord.Client):
    ...
    async def cmd_remove(self, player, message, channel, author, leftover_args):
        ...
        try:
            ...
            for entry in player.playlist.entries:
                self.safe_print("Looking at {0}. [{1}]".format(
    ...
    async def cmd_playlist(self, channel, author, server, player, leftover_args):
        """
        Usage:
            {command_prefix}playlist showall [alphabetical, author, entries, playtime, random]
            {command_prefix}playlist savename
            {command_prefix}playlist save savename
            {command_prefix}playlist load savename [add, replace] [startindex, endindex (inclusive)]
        ...
        if argument == "save":
            ...
            sort_modes = {"alphabetical": (lambda playlist: playlist, False), "entries": (lambda playlist: int(
                self.playlists.get_playlist(playlist, player.playlist)["entry_count"]), True), "author": (lambda playlist: server.get_member(self.playlists.get_playlist(playlist, player.playlist)["author"]).name, False), "random": None, "playtime": (lambda playlist: sum([x.duration for x in self.playlists.get_playlist(playlist, player.playlist)["entries"]]), True)}
            ...
            response_text = "\"{}\" added by *{}* with {} entr{}\n*playtime: {}*\n\n{}\n```\nTo edit this playlist type \"{}playlist builder {}\"```".format(argument.title(), server.get_member(
