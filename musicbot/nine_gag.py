...
def retrieve_articles(number_of_pages, page_type):
  ...
  while number_of_pages > 0:
    ...
    if content == None:
      return None
    extend_url = content.find('a', attrs=***REMOVED***'class' : 'btn badge-load-more-post'***REMOVED***)['href']
    #print extend_url
...
def annotate(number_of_pages, page_type):
  ...
  for ii in retrieve_articles(number_of_pages, page_type):
    ...
    try:
      type = ii.find('span', attrs = ***REMOVED***'class' : 'play badge-gif-play hide'***REMOVED***).text
      media_url = ii.find('img', attrs = ***REMOVED***'class' : 'badge-item-animated-img'***REMOVED***)['src']
      file_format = ".gif"
    except:
        try:
            type = "video/mp4"
            media_url = ii.find('source', attrs = ***REMOVED***'type' : 'video/mp4'***REMOVED***)['src']
            file_format = ".mp4"
        except:
            type = 'Image'
            media_url = ii.find('img', attrs = ***REMOVED***'class' : 'badge-item-img'***REMOVED***)['src']
            file_format = ".jpg"
