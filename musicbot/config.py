...
class Config:
    def __init__(self, config_file):
        ...
        confsections = ***REMOVED***"Credentials", "Permissions", "Chat", "MusicBot"***REMOVED***.difference(config.sections())
        if confsections:
            raise HelpfulError(
                "One or more required config sections are missing.",
                "Fix your config.  Each [Section] should be on its own line with "
                "nothing else on it.  The following sections are missing: ***REMOVED******REMOVED***".format(
