...
class PatchedBuff:
    ...
    def read(self, frame_size):
        ...
        if self.draw and not self.frame_count % self.frame_skip:
            ...
            meter_text = 'avg rms: ***REMOVED***:.2f***REMOVED***, max rms: ***REMOVED***:.2f***REMOVED*** '.format(self._avg(self.rmss), max_rms)
    ...
    def _pprint_meter(self, perc, *, char='#', text='', shift=True):
        ...
        if shift:
            outstr = text + "***REMOVED******REMOVED***".format(char * (int((tx - len(text)) * perc) - 1))
        else:
            outstr = text + "***REMOVED******REMOVED***".format(char * (int(tx * perc) - 1))[len(text):]
