...
class PlaylistCommands:
    ...
    ***REMOVED***)
    ...
    async def cmd_editentry(self, channel, author, player, leftover_args):
        ...
        playlistname = (
            "_".join(leftover_args) or entry.meta.get("playlist", ***REMOVED******REMOVED***).get("name", None)
        ).lower().strip()
        ...
        if new_entry:
            ...
            return Response("Didn't save *****REMOVED******REMOVED***** from `***REMOVED******REMOVED***`".format(new_entry.title, playlistname.replace("_", " ").title()))
