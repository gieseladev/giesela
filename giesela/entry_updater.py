...
async def _rebuild_entry(queue, entry):
    try:
        ...
        if entry["type"] == "GieselaEntry":
            print("[Entry-Fixer] \"{}\"({}) had GieselaEntry information in it, applying this information".format(
                entry.get("title", "Unknown title")[:25], entry.get("url", "Unknown URL")))
...
def _convert_spotify_to_giesela(queue, entry):
    ...
    return GieselaEntry(queue, video_id, url, title, duration, thumbnail, description, song_title, artist, artist_image, album, cover,
                        expected_filename=expected_filename, **meta).to_dict()
...
async def fix_entry(queue, entry):
    ...
    if version < 100:
        print("[Entry-Fixer] \"{}\"({}) is way too old, gotta rebuild completely!".format(entry.get("title", "Unknown title")[:25],
                                                                                          entry.get("url", "Unknown URL")))
        return await _rebuild_entry(queue, entry)

    if entry.get("spotify_data", {}).get("id", None) == "custom":
        print("[Entry-Fixer] \"{}\"({}) is an old spotify entry, converting to the new GieselaEntry!".format(entry.get("title", "Unknown title")[:25],
                                                                                                             entry.get("url", "Unknown URL")))
        entry = _convert_spotify_to_giesela(queue, entry)

    if entry.get("broken"):
        print("[Entry-Fixer] \"{}\"({}) has been marked as broken, trying to recover".format(entry.get("title", "Unknown title")[:25],
                                                                                             entry.get("url", "Unknown URL")))
        try:
            ...
            if (not data) or data.get("_type", None) == "playlist":
                print("[Entry-Fixer] \"{}\"({}) no data returned by youtube_dl... or it's a playlist".format(entry.get("title", "Unknown title")[:25],
                                                                                                             entry.get("url", "Unknown URL")))
                return None

            entry["version"] = Entry.version
            entry.pop("broken", None)
            print("[Entry-Fixer] \"{}\"({}) got some data, creating the new entry!".format(entry.get("title", "Unknown title")[:25],
                                                                                           entry.get("url", "Unknown URL")))
            return Entry.from_dict(queue, entry)
        except Exception:
            print("[Entry-Fixer] \"{}\"({}) something is wrong with this entry, youtube_dl raised an exception!".format(
                entry.get("title", "Unknown title")[:25], entry.get("url", "Unknown URL")))
            return None

    if not entry.get("expected_filename", False):
        print("[Entry-Fixer] \"{}\"({}) doesn't have \"expected_filename\" (needed for caching), fixing!".format(
            entry.get("title", "Unknown title")[:25], entry.get("url", "Unknown URL")))
        return await _fix_filename(queue, entry)

    try:
        print("[Entry-Fixer] \"{}\"({}) no idea what's wrong, setting to newest version, removing broken flag".format(
            entry.get("title", "Unknown title")[:25], entry.get("url", "Unknown URL")))
        entry["version"] = Entry.version
        entry.pop("broken", None)
        return Entry.from_dict(queue, entry)
