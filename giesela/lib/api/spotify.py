...
class SpotifyAlbum:
    ...
    def from_data(cls, data):
        return cls(data["id"], data["name"], [SpotifyArtist.from_data(artist, simple=True) for artist in data["artists"]], data["images"],
                   data["uri"])
...
class SpotifyTrack:
    ...
    def from_dict(cls, data):
        ...
        return cls(data["id"], data["name"], [SpotifyArtist.from_dict(artist) for artist in data["artists"]] if data["artists"] is not None else None,
                   data["duration"], SpotifyAlbum.from_dict(data["album"]) if data["album"] is not None else None, data["popularity"], data["uri"],
                   data["query"], data["certainty"])
...
def get_certainty(query, song_name, artist_name):
    ...
    poss = [similarity(query.lower(), "{0} {1}".format(song_name_edited, artist_name)),
            similarity(query.lower(), "{1} {0}".format(song_name_edited, artist_name))]
