...
from giesela.utils import prettydate
...
class GameCAH:
    ...
    def __init__(self, giesela):
        self.giesela = giesela
        self.running_games = {}
    ...
    def send_message_to_user(self, user_id, message, callback=None, delete_after=0):
        user = self.giesela.get_global_user(user_id)

        if user is None:
            return None

        task = self.giesela.loop.create_task(
            self.giesela.safe_send_message(user, message, expire_in=delete_after))
    ...
    def wait_for_message(self, callback, timeout=None, check=None):
        ...
        # print(";;;;;;;;;;;;;;;;;;" + str(callback))
        task = self.giesela.loop.create_task(
            self.giesela.wait_for_message(timeout=timeout, check=check))
        task.add_done_callback(callback)

    def delete_message(self, message):
        self.giesela.loop.create_task(
            self.giesela.safe_delete_message(message))
...
class Game:
    ...
    def add_user(self, user_id):
        ...
        self.broadcast("*" + self.manager.giesela.get_global_user(
    ...
    def send_player_stats(self, player):
        ...
        player_stats.append(player_stats_interface.format(self.game.manager.giesela.get_global_user(
...
class Round:
    ...
    def __init__(self, game, round_index):
        ...
        self.game.broadcast("***{}** is the master this round*".format(
            self.game.manager.giesela.get_global_user(self.master.player_id).name), delete_after=60)
    ...
    def on_player_message(self, player, message, round_index):
        ...
        if args[0] == "pick" or num is not None:
            ...
            self.game.manager.send_message_to_user(
                player.player_id, "Card **{0.id}** by {1}\n```\n\"{0.text}\"\nused {0.occurences} time{2}\ndrawn {0.picked_up_count} time{5}\nlike ratio: {4}%\ncreated {3}```".format(card_chosen, self.game.manager.giesela.get_global_user(card_chosen.creator_id).name, "s" if card_chosen.occurences != 1 else "", prettydate(card_chosen.creation_date), int(card_chosen.like_dislike_ratio * 100), "s" if card_chosen.picked_up_count != 1 else ""), delete_after=20)
            wait_again()
            return
        elif args[0] == "like":
            try:
    ...
    def master_picks(self, index):
        ...
        self.game.broadcast("**{}** won the game with the card{} {}".format(self.game.manager.giesela.get_global_user(player_key.player_id).name, "s" if len(
        ...
        for pl in self.game.players:
            if pl == self.master:
                ...
                self.game.manager.send_message_to_user(pl.player_id, player_judge_text.format(
                    self.question_card.beautified_text(), self.question_card.id, "\n".join(answer_texts), self.game.manager.giesela.get_global_user(self.master.player_id).name), callback=lambda x: self.messages_to_delete.append(x.result()))
    ...
    def on_master_message(self, player, message, round_index):
        ...
        if self.judging_phase and (args[0] == "pick" or card_index is not None):
            ...
            self.game.manager.send_message_to_user(
                player.player_id, "Card **{0.id}** by {1}\n```\n\"{0.text}\"\nused {0.occurences} time{2}\ndrawn {0.picked_up_count} time{5}\nlike ratio: {4}%\ncreated {3}```".format(self.question_card, self.game.manager.giesela.get_global_user(self.question_card.creator_id).name, "s" if self.question_card.occurences != 1 else "", prettydate(self.question_card.creation_date), int(self.question_card.like_dislike_ratio * 100), "s" if self.question_card.picked_up_count != 1 else ""), delete_after=20)
            wait_again()
            return
        elif args[0] == "stats":
            self.game.send_player_stats(player)
