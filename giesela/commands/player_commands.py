...
from giesela.utils import (Response, command_info, create_bar,
                           parse_timestamp)


async def cmd_repeat(player):
    """
    ///|Usage
...
class PlayerCommands:
    ...
    async def cmd_volume(self, player, leftover_args):
        """
        ///|Usage
        ...
        if relative:
            ...
            vol_change = new_volume - player.volume
        ...
        if 0 <= new_volume <= 100:
            ...
            if relative:
                raise exceptions.CommandError(
                    "Unreasonable volume change provided: {}{:+} -> {}%.  Provide a change between {} and {:+}.".format(old_volume, vol_change,
