...
class EnqueueCommands:
    ...
    async def cmd_radio(self, player, channel, author, leftover_args):
        ...
        if leftover_args:
            if leftover_args[0].lower().strip() == "random":
                ...
                station_info = RadioStations.get_station(search_name.lower().strip())
                if station_info:
                    await player.queue.add_radio_entry(station_info, channel=channel, author=author, now=True)
    ...
    async def cmd_search(self, player, channel, author, leftover_args):
        ...
        while True:
            ...
            result_message = await self.safe_send_message(channel, result_string.format(current_result_index + 1, total_results,
                                                                                        current_result["webpage_url"]))
            ...
            response_message = await self.wait_for_message(100, author=author, channel=channel,
                                                           check=lambda msg: msg.content.strip().lower().split()[0] in (
                                                               "play", "addtoplaylist", "n", "p", "exit"))
    ...
    async def cmd_spotify(self, message, channel, author, player, url):
        ...
        if isinstance(model, spotify.SpotifyTrack):
            ...
            await self.edit_message(interface_msg, "**Loaded playlist**", embed=em)
...
class DisplayCommands:
    ...
    async def cmd_np(self, player, channel, server):
        """
        ///|Usage
        {command_prefix}np
        ///|Explanation
    ...
    async def cmd_queue(self, player, num="15"):
        """
        ///|Usage
    ...
    async def cmd_history(self, player, num="15"):
        """
        ///|Usage
