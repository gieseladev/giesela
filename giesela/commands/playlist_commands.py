...
class PlaylistCommands:
    ...
    async def cmd_playlist(self, channel, author, player, leftover_args):
        """
        ///|Load
    ...
    async def playlist_builder(self, channel, author, player, _savename):
        ...
        while (not abort) and (not save):
            ...
            for i in range(start, end):
                entries_text += "`{:0>2}.` {} {}\n".format(i + 1, nice_cut(entries[i].title, 50),
                                                           self.config.entry_type_emojis.get(entries[i].__class__.__name__, ""))
            ...
            if split_message[0].lower() == "add":
                ...
                if not resp.content.lower().startswith(self.config.command_prefix) and not resp.content.lower().startswith("abort"):
                    ...
                    if cmd == "removeduplicates":
                        ...
                        rebuild_safe_entries, rebuild_removed_entries = await _get_entries_from_urls(entry_urls,
                                                                                                     "Rebuilding the playlist. This might take a while, please hold on.")
        ...
        if save:
            if any((
                    pl_changes["added_entries"], pl_changes["remove_entries"], pl_changes["new_name"], pl_changes["new_desc"],
                    pl_changes["new_cover"],
                    pl_changes["changed_entries"])):
                c_log = "**CHANGES**\n\n"
    ...
    async def entry_manipulator(self, player, channel, author, playlist_name, entry):
        ...
        while True:
            ...
            response = await self.wait_for_message(timeout=None, author=author, channel=channel,
                                                   check=lambda msg: msg.content.lower().strip().startswith(
                                                       ("set", "remove", "timestamp", "exit", "abort", "save")))
                    ...
                    if rest:
                        if property_target == "sub_queue":
                            ...
                            if property_target in ("cover_url", "thumbnail", "artist_image_url"):
                                if is_image(rest):
                                    url = await upload_song_image(self.loop, playlist_name,
                                                                  "{} {}".format(entry_fields["_video_id"], property_target), rest)
                                    if url:
    ...
    async def cmd_addtoplaylist(self, channel, author, player, playlistname, leftover_args):
        ...
        if res:
            ...
            response = await self.wait_for_message(timeout=60, channel=channel, author=author,
                                                   check=lambda msg: msg.content.lower().strip() in ["y", "yes", "no", "n"])
    ...
    async def cmd_removefromplaylist(self, player, playlistname=None):
        """
        ///|Usage
        ...
        playlist_data = self.playlists.get_playlist(playlist_id, player.queue, False)
