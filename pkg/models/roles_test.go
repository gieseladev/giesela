...
func testRoleToManyRoleTargets(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = randomize.Struct(seed, &b, roleTargetDBTypes, false, roleTargetColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	if err = randomize.Struct(seed, &c, roleTargetDBTypes, false, roleTargetColumnsWithDefault...); err != nil {
		t.Fatal(err)
	}
	...
	check, err := a.RoleTargets().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadRoleTargets(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RoleTargets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.RoleTargets = nil
	if err = a.L.LoadRoleTargets(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.RoleTargets); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRoleToManyAddOpRoleTargets(t *testing.T) {
	var err error
	...
	foreigners := []*RoleTarget{&b, &c, &d, &e}
	for _, x := range foreigners {
		if err = randomize.Struct(seed, x, roleTargetDBTypes, false, strmangle.SetComplement(roleTargetPrimaryKeyColumns, roleTargetColumnsWithoutDefault)...); err != nil {
			t.Fatal(err)
		}
	}
	...
	foreignersSplitByInsertion := [][]*RoleTarget{
		{&b, &c},
		{&d, &e},
	}

	for i, x := range foreignersSplitByInsertion {
		err = a.AddRoleTargets(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.RoleTargets[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.RoleTargets[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.RoleTargets().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
var (
	roleDBTypes = map[string]string{`ID`: `bigint`, `GuildID`: `text`, `Name`: `text`, `Permissions`: `ARRAYsmallint`}
	_           = bytes.MinRead
)
