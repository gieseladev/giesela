...
var RoleWhere = struct {
	...
	GuildID:     whereHelpernull_String{field: "\"roles\".\"guild_id\""},
	Name:        whereHelperstring{field: "\"roles\".\"name\""},
	Permissions: whereHelpertypes_Int64Array{field: "\"roles\".\"permissions\""},
}
...
var (
	roleAllColumns            = []string{"id", "guild_id", "name", "permissions"}
	roleColumnsWithoutDefault = []string{"guild_id", "name", "permissions"}
...
func (o *Role) RoleTargets(mods ...qm.QueryMod) roleTargetQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"role_targets\".\"role_id\"=?", o.ID),
	)

	query := RoleTargets(queryMods...)
	queries.SetFrom(query.Query, "\"role_targets\"")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"\"role_targets\".*"})
	}

	return query
}

// LoadRoleTargets allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadRoleTargets(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(qm.From(`role_targets`), qm.WhereIn(`role_targets.role_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err != nil {
		return errors.Wrap(err, "failed to eager load role_targets")
	}

	var resultSlice []*RoleTarget
	if err = queries.Bind(results, &resultSlice); err != nil {
		return errors.Wrap(err, "failed to bind eager loaded slice role_targets")
	}

	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on role_targets")
	}
	if err = results.Err(); err != nil {
		return errors.Wrap(err, "error occurred during iteration of eager loaded relations for role_targets")
	}
	...
	if singular {
		object.R.RoleTargets = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
				foreign.R = &roleTargetR{}
			}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.RoleID {
				local.R.RoleTargets = append(local.R.RoleTargets, foreign)
				if foreign.R == nil {
					foreign.R = &roleTargetR{}
				}
...
// AddRoleTargets adds the given related objects to the existing relationships
// of the role, optionally inserting them as new records.
// Appends related to o.R.RoleTargets.
...
func (o *Role) AddRoleTargets(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*RoleTarget) error {
	...
	if o.R == nil {
		...
		o.R.RoleTargets = append(o.R.RoleTargets, related...)
	}
